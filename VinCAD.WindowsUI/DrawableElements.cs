...
namespace VinCAD.WindowsUI
{
    ...
    public class Line : IDrawable, ISelectable, IDisposable
    {
        ...
        {
            using (var path = Path)
                return path.IsOutlineVisible(p, new Pen(Color.Black, SelectDelta));
        }
...

            {
                ...
                for (int i = 0; i < _segments.Count; ++i)
                    ...
                    else if (_segments[i].Direction == Direction.Y)
                    {
                        ...
                        if (Math.Abs(segmentPoint.X - prevX) <= SelectDelta / 2
                            && (_segments[i].Length >= 0 ? delta <= _segments[i].Length && delta > 0 : delta >= _segments[i].Length && delta < 0))
                        {
                            segmentIndex = i;
                            break;
                ...
                switch (direction)
                {
                    case Direction.X:
                        if (dy != 0)
                            ...
                            {
                                _segments[0].Length -= _end.Width + 1;
                                _segments.Add(new LineSegment(Direction.Y, dy));
                                _segments.Add(new LineSegment(Direction.X, dx - _segments[0].Length));
                            }
                        break;
                    case Direction.Y:
                        if (dx != 0)
