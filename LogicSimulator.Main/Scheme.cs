...
namespace LogicSimulator.Main
{
    public class Scheme
    {
        ...
        public void SetCurrentState(IEnumerable<bool> state)
        {
        ...
        public void SetCurrentState(int stateNumber)
        {
            ...
            var state = new bool[_inputs.Count];

            for (int i = 0; i < state.Length; i++)
                state[state.Length - i - 1] = ((stateNumber >> i) & 1) == 1;
        ...
        public void CalculateForCurrentState()
        {
            foreach (var layer in ComponentLayers)
                ...
                {
                    var values = new bool[component.Input.Count];
        ...
        public bool GetElementValue(string name)
        {
        ...
        public bool[,] CalculateTable()
        {
            ...
            var table = new bool[inputs_count_pow2, inputs_plus_outputs_count];
            ...
            for (int i = 0; i < inputs_count_pow2; i++)
            {
                ...
                {
                    if (j < _inputs.Count)
                        table[i, j] = _inputs[j].Value.Value;
                    else
                        table[i, j] = _outputs[j - _inputs.Count].Value.Value;
                }
            }
        ...
        public void WriteTable(Stream stream, bool[,] table)
        {
            #region arguments check
            ...
            using (var writer = new StreamWriter(stream))
            {
                ...
                for (int i = 0; i < table_Count[0]; i++)
                {
                    ...
                    for (int j = 0; j < table_Count[1]; j++)
                        writer.Write($"<td{(j == _inputs.Count ? " class='border-left'" : string.Empty)}>{Convert.ToInt16(table[i,j])}</td>");
                    writer.Write( "</tr>");
                }
